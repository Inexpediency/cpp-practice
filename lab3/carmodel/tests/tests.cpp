...
struct CarFixture
{
	...
	void ExpectOperationFailure(const std::function<bool(CCar & car)> & operation)const
	{
		...
		BOOST_CHECK_EQUAL(clone.GetDirection(), car.GetDirection());
	}
...
BOOST_FIXTURE_TEST_SUITE(Car_create, CarFixture)
	BOOST_AUTO_TEST_CASE(engine_turn_off)
	{
